...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog = 'MARSHAL CODE EXTRACTOR',
        description = 'Attempts to Extract Marshal Code From PyObfuscate obfuscation.'
    )
    ...
    while True:
        ...
        if user_in.upper() == 'Y':
            print('τнis τσσℓ ωαs ɒєcσɒєɒ вy cмɒ αмєя')
            with open('KILLED.py', 'wb') as kill:
                    kill.write(b"import marshal\nexec(marshal.loads(" + repr(decoded_layer).encode('utf-8') + b"))")
                    break
